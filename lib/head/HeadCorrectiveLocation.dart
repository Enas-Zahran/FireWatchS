...
class HeadCorrectiveLocationsPage extends StatefulWidget {
  ...
  @override
  State<HeadCorrectiveLocationsPage> createState() => _HeadCorrectiveLocationsPageState();
}

class _HeadCorrectiveLocationsPageState extends State<HeadCorrectiveLocationsPage> {
 ...
 Future<void> _loadData() async {
  try {
    ...
    final tasks = await supabase
        ...
        .select('id, tool_id, status, safety_tools(id, name, type, location_id)');
  ...
  List<Map<String, dynamic>> _tasksForLocation(String code) {
    return correctiveTasks.where((task) {
      final match = (task['location_code'] ?? '').toString().trim().toUpperCase() == code.trim().toUpperCase();
  ...
  void _navigateToReviewPage(BuildContext context, Map<String, dynamic> task) async {
    ...
    if (toolType == 'fire extinguisher') {
      await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => FireExtinguisherHeadReviewPage(
            taskId: taskId,
            toolName: toolName,
            headName: headName,
            taskType: 'علاجي',
      ...
      await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => FireHydrantHeadReviewPage(
            taskId: taskId,
            toolName: toolName,
            headName: headName,
            taskType: 'علاجي',
      ...
      await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => HoseReelHeadReviewPage(
            taskId: taskId,
            toolName: toolName,
            headName: headName,
            taskType: 'علاجي',
  ...
  Widget build(BuildContext context) {
    return Directionality(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('مراجعة المهام العلاجية', style: TextStyle(color: Colors.white)),
          ...
          actions: [
            Row(
              children: [
                const Text('عرض المعتمدة', style: TextStyle(color: Colors.white)),
                Switch(
                  value: showApproved,
                ...
                itemBuilder: (context, index) {
                  ...
                  return Card(
                    ...
                    child: ExpansionTile(
                      ...
                      children: tasksInLocation.isEmpty
                          ? [
                              const Padding(
                              ...
                              return ListTile(
                                title: Text(
                                  ...
                                  style: TextStyle(
                                    ...
                                    fontWeight: isReviewed ? FontWeight.bold : FontWeight.normal,
                                  ),
                                ),
                                ...
                                onTap: () => _navigateToReviewPage(context, task),
                              );
                            }).toList(),
                    ),
                  );
