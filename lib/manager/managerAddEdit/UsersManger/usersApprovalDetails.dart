  ...
  Future<void> _approveUser() async {
    if (_formKey.currentState!.validate()) {
      try {
        ...
        // ✅ Update with .select() to confirm result
        ...
        if (response == null || response.isEmpty) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('فشل في تحديث المستخدم. تحقق من الصلاحيات أو البيانات.')),
          );
          return;
        }
        ...
        Navigator.of(context).pop(true); // ✅ Return to refresh previous page
      } catch (e) {
        ScaffoldMessenger.of(context).showSnackBar(
