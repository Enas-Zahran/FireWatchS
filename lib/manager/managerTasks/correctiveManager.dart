...
class _CorrectiveTasksPageState extends State<CorrectiveTasksPage> {
  ...
  Future<void> _assignTasks() async {
    ...
    try {
      ...
      final toolDetails =
          reports.where((r) => selectedReportIds.contains(r['id'])).toList();
      ...
      for (final report in toolDetails) {
        ...
        final matchingTool =
            await supabase
                ...
                .eq('name', toolName) // ✅ toolName هنا مضمونة أنها ليست null
                .maybeSingle();
      ...
      final updatedTaskCount =
          (userData['task_count'] ?? 0) + selectedReportIds.length;
  ...
  Widget build(BuildContext context) {
    ...
    final filteredReports =
        reports.where((report) {
          return report['tool'].toString().toLowerCase().contains(
                keyword.toLowerCase(),
              ) ||
              (report['locationName'] ?? '').toString().startsWith(
                keyword.toLowerCase(),
              );
        }).toList();
            ...
            _isLoading
                ...
                : Padding(
                  ...
                  child: Column(
                    children: [
                      Text(
                        'عدد البلاغات المحددة: ${selectedReportIds.length} من ${filteredReports.length} ($selectedRatio)',
                      ),
                                  ...
                                  return ListTile(
                                    tileColor:
                                        assignedToAnother
                                            ...
                                            : null,
                                    title: Text(
                                      '${report['tool']} - ${report['locationName']}',
                                    ),
                                    subtitle: Column(
                                      crossAxisAlignment:
